...
classDiagram
    ...
    class OrderProcessor {
        ...
        +processOrder(Order order, String fileName, String email)
    }

    class DatabaseOrderPlacer {
    }
    ...
    interface InvoiceGenerator {
        <<interface>>
        +generateInvoice(Order order, String fileName)
    }
    interface NotificationSender {
        <<interface>>
        +sendEmailNotification(Order order, String email)
    }

    OrderProcessor --> OrderPlacer : depends on
    OrderProcessor --> InvoiceGenerator : depends on
    OrderProcessor --> NotificationSender : depends on
    OrderProcessor --> Order : uses

    DatabaseOrderPlacer ..|> OrderPlacer : implements
    PdfInvoiceGenerator ..|> InvoiceGenerator : implements
